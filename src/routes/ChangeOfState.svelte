<script>
    ...
    "reversible": {value: true, disabled: true}};
    let variables = {};
    let variableInput = "";
    const options = {
        Q: {value: NaN, unit: "J"},
    ...
    function handleKeypress(event) {
        if (event.keyCode === 13) {
            let [variable,value] = variableInput.split("=").map(part => part.trim())
            if (attributeOptions.includes(variable)) {
            ...
            <div class="table">
                {#each Object.entries(variables) as [variable,props]}
                    ...
                    {#if variable in options}
                        <label class="unit">{@html options[variable].unit}</label>
                    {:else}
...
-->
    <div class="item add">
        <label>+</label> 
        <input type="text" class="stretch-width" placeholder="Enter a variable..." on:keypress={handleKeypress} bind:value={variableInput}/>
